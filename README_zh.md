...
- **结构化几何库**: 支持低维度的几何族，包括直楔形 (wedge)、阶梯形 (step)、双层桥接 (double-layer)、曲线楔形 (curved wedge)、拱形 (arc) 以及面内平面分割 (planar split)。这使得数据库能够由具有明确物理意义和可解释性的结构主导，而不仅仅是随机平滑噪声。其中 `planar_split` 让两种材料各自贯穿全厚度、由竖直界面在面内左右或对角分开，是唯一顶面不被低热导层整体覆盖的族，因此能达到给定厚度下的最大顶面对比度。
