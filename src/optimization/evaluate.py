...
def _delta_range_metrics(predictions):
    ranges = [
        ("<=10K", -np.inf, 10.0),
        ("10-15K", 10.0, 15.0),
        ("15-50K", 15.0, 50.0),
    ...
    for label, low, high in ranges:
        mask = (predictions["delta_T_true"] > low) & (predictions["delta_T_true"] <= high)
        subset = predictions[mask]
        if subset.empty:
            rows.append({"delta_T_range": label, "count": 0})
            continue
        y_true = subset["delta_T_true"].to_numpy()
        y_pred = subset["delta_T_pred"].to_numpy()
        metrics = _metrics(y_true, y_pred)
        metrics["delta_T_range"] = label
        metrics["mean_abs_relative_error"] = float(
            np.mean(np.abs(y_pred - y_true) / np.clip(np.abs(y_true), 1.0, None))
        )
        rows.append(metrics)
...
def evaluate_model(args):
    ...
    per_delta_range = _delta_range_metrics(predictions)
    ...
    per_delta_range_path = os.path.join(output_dir, f"per_delta_range_metrics_{args.split}.csv")
    ...
    per_delta_range.to_csv(per_delta_range_path, index=False)
    ...
    print("\nDelta_T range metrics:")
    print(per_delta_range.to_string(index=False, float_format=lambda value: f"{value:.6g}"))
    print("\nPer-family metrics:")
