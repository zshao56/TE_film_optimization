...
def run_pipeline(config_path, stages=None):
    config_path = Path(config_path).resolve()
    config = _load_config(config_path)
    stages = set(stages or [])

    def should_run(stage):
        return (not stages or stage in stages) and _stage_enabled(config, stage)

    if should_run("data_generation"):
        run_data_generation(config, config_path)
    if should_run("metadata_filter"):
        run_metadata_filter(config)
    if should_run("training"):
        run_training(config)
    if should_run("evaluation"):
        run_evaluation(config)
    if should_run("real_world_benchmark"):
...
def build_parser():
    ...
    parser.add_argument(
        ...
        help="Optional subset of stages to run. Defaults to all enabled stages in the config.",
    )
    return parser
