...
def _sample_benchmark_candidate(args, scenario, seed, candidate_index):
    ...
    sampling_config = getattr(args, "sampling_config", {}) or {}
    h = _sample_thickness(rng, "expanded", sampling_config)
    k_low, k_high = _sample_materials(rng, "expanded", sampling_config)
...
def _apply_config(args):
    ...
    setattr(
        args,
        "sampling_config",
        benchmark_config.get("sampling", config.get("data_generation", {}).get("sampling", {})),
    )
    for key, value in benchmark_config.items():
        if key == "scenarios":
            setattr(args, "scenarios", value)
        elif key == "sampling":
            setattr(args, "sampling_config", value)
